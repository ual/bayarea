...
def buildings(store):
    ...
    # Fill missing values in: residential_units, residential_sqft and improvement_value in res_buildings
    parcels = orca.get_table('parcels').to_frame(['zone_subdistrict_id'])
    df['zone_subdistrict_id'] = misc.reindex(parcels.zone_subdistrict_id,
                                                    df.parcel_id)

    idx_residential = df.building_type_id.isin([1110, 1121, 1122, 1130, 1210, 1220])
    idx_mixeduse = df.building_type_id.isin([2111, 2112, 2121, 2122, 2131, 2132, -1])
    # -1 has res and non res sqft son we sum them

    # Rows with complete res information to be used to calculate averages
    df_rescomplete = df[(idx_residential) & (df.residential_units > 0) & (df.residential_sqft > 0)]
    df_rescomplete['sqft_per_unit'] = df_rescomplete.residential_sqft / df_rescomplete.residential_units
    # Calculate average by building type and zone subdistrict
    mean_res_byzone = df_rescomplete.groupby(['zone_subdistrict_id', 'building_type_id']).mean()[['residential_units',
                                                                                                'residential_sqft', 'sqft_per_unit']].reset_index()
    # Case A: Res buildings with no information in residential_units AND residential_sqft
    for i, row in df[(idx_residential) & (df.residential_units == 0) &
                            (df.residential_sqft == 0)].iterrows():
        b_id = row.building_type_id
        zone_sub = row.zone_subdistrict_id
        matching_zone_type = (mean_res_byzone.zone_subdistrict_id == zone_sub) & (mean_res_byzone.building_type_id == b_id)
        # Grabbing information from non_residential_sqft if > 0 for the residential_sqft
        if row.non_residential_sqft > 0:
            df.loc[i, 'residential_sqft'] = row.non_residential_sqft
            df.loc[i, 'non_residential_sqft'] = 0
            #For residential_units look for the building_type and zone_subdistrict id average
            if len(mean_res_byzone[matching_zone_type]) > 0:
                df.loc[i, 'residential_units'] = mean_res_byzone[matching_zone_type]['residential_units'].values[0]
            else:
                df.loc[i, 'residential_units'] = mean_res_byzone.loc[mean_res_byzone.building_type_id == b_id,
                                                                            'residential_units'].mean()
        else:
            if len(mean_res_byzone[matching_zone_type]) > 0:
                df.loc[i, 'residential_units'] = mean_res_byzone[matching_zone_type]['residential_units'].values[0]
                df.loc[i, 'residential_sqft'] = mean_res_byzone[matching_zone_type]['residential_sqft'].values[0]
            else:
                df.loc[i, 'residential_units'] = mean_res_byzone.loc[mean_res_byzone.building_type_id == b_id,
                                                                                'residential_units'].mean()
                df.loc[i, 'residential_sqft'] = mean_res_byzone.loc[mean_res_byzone.building_type_id == b_id,
                                                                                'residential_sqft'].mean()
    # Case B: Res buildings with no information in residential_sqft but with residential_units
    for i, row in df[(idx_residential) & (df.residential_units > 0) &
                                (df.residential_sqft == 0)].iterrows():
        b_id = row.building_type_id
        zone_sub = row.zone_subdistrict_id
        ...
        if row.non_residential_sqft > 0:
            ...
            if len(mean_res_byzone[matching_zone_type]) > 0:
                ...
                df.loc[i, 'residential_sqft'] = mean_res_byzone.loc[mean_res_byzone.building_type_id == b_id,
                                                                                'residential_sqft'].mean()
    # Case C: Res buildings with no information in residential_units but with residential_sqft
    for i, row in df[(idx_residential) & (df.residential_units == 0) &
                                (df.residential_sqft > 0)].iterrows():
        b_id = row.building_type_id
        zone_sub = row.zone_subdistrict_id
        matching_zone_type = (mean_res_byzone.zone_subdistrict_id == zone_sub) & (mean_res_byzone.building_type_id == b_id)
        if len(mean_res_byzone[matching_zone_type]) > 0:
            df.loc[i, 'residential_units'] = np.ceil(row.residential_sqft / mean_res_byzone[matching_zone_type]['sqft_per_unit'].values[0])
        else:
            df.loc[i, 'residential_units'] = np.ceil(row.residential_sqft / mean_res_byzone.loc[mean_res_byzone.building_type_id == b_id, 'sqft_per_unit'].mean())

    # Fill improvement_values == 0 in res buildings
    df['units'] = 0
    df.units[idx_residential] = df.residential_units[idx_residential]
    df.units[idx_mixeduse] = df.residential_sqft[idx_mixeduse] + df.non_residential_sqft[idx_mixeduse]
    df.units[(~idx_mixeduse) & (~idx_residential)] = df.non_residential_sqft[(~idx_mixeduse) & (~ idx_residential)]
    
    # Calculate impval_per_unit
    df['impval_per_unit'] = (df.improvement_value / df.units)
    
    impvalue_mean = df[(df.improvement_value != 0) & (df.units > 0)].groupby(['zone_subdistrict_id',
                                                                    'building_type_id']).agg({'impval_per_unit': 'mean'})
    impvalue_mean.reset_index(inplace=True)

    #Function looks up in impvalue_mean table according to the building type and zone_subdistrict
    def impmean(x):
        if len(impvalue_mean.loc[(impvalue_mean.zone_subdistrict_id == x[0]) & (impvalue_mean.building_type_id == x[1])]) > 0:
            return impvalue_mean.loc[(impvalue_mean.zone_subdistrict_id == x[0]) & (impvalue_mean.building_type_id == x[1]),
                                        'impval_per_unit'].mean()
        else:
            return impvalue_mean.loc[impvalue_mean.building_type_id == x[1], 'impval_per_unit'].mean()

    # Fill with mean value per sqft by subdistrict and by building type
    df.loc[(df.improvement_value == 0) & (idx_residential), 'impval_per_unit'] = df[(df.improvement_value == 0) & (idx_residential)][['zone_subdistrict_id', 'building_type_id']].apply(impmean, axis=1)
    df.loc[(df.improvement_value == 0) & (idx_residential), 'improvement_value'] = df['units'] * df['impval_per_unit']

    df.impval_per_unit.fillna(0, inplace=True)
    df.impval_per_unit.replace([np.inf, -np.inf], 0, inplace=True)
    df.drop(columns='zone_subdistrict_id', inplace=True)
